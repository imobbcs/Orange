...
async function storePriceSnapshot(price: number) {
  await pool.query(
    'INSERT INTO btc_price_history (price_eur) VALUES ($1)',
    [price]
  );
  // Keep only last 7 days of snapshots
  await pool.query(
    "DELETE FROM btc_price_history WHERE recorded_at < NOW() - INTERVAL '7 days'"
  );
}
...
async function getLastAlertTime(): Promise<Date | null> {
  const { rows } = await pool.query(
    'SELECT fired_at FROM alert_log ORDER BY fired_at DESC LIMIT 1'
  );
...
function buildNotification(price: number, movePct: number, direction: 'up' | 'down', lang: 'en' | 'de') {
  ...
  if (lang === 'de') {
    return direction === 'down'
      ? {
          heading: `Bitcoin fiel ${pct}% in 4 Stunden`,
          message: `BTC steht bei ${fmt(price)}. Bitcoiner kaufen.`,
        }
      : {
          heading: `Bitcoin stieg ${pct}% in 4 Stunden`,
          message: `BTC steht bei ${fmt(price)}. Bitcoiner halten — und verkaufen nicht in den Hype hinein.`,
        };
  }

  return direction === 'down'
    ? {
        heading: `Bitcoin dropped ${pct}% in 4 hours`,
        message: `BTC is now ${fmt(price)}. Bitcoiners are buying.`,
      }
    : {
        heading: `Bitcoin surged ${pct}% in 4 hours`,
        message: `BTC is now ${fmt(price)}. Bitcoiners are holding — and not selling into the hype.`,
      };
}

async function sendPush(price: number, movePct: number, direction: 'up' | 'down') {
  // Send two notifications: one for EN subscribers, one for DE subscribers
  // Untagged subscribers (no lang tag) fall back to EN via the EN notification
  const sends = [
    {
      lang: 'en' as const,
      filters: [{ field: 'tag', key: 'lang', relation: '!=', value: 'de' }],
    },
    {
      lang: 'de' as const,
      filters: [{ field: 'tag', key: 'lang', relation: '=', value: 'de' }],
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // Only allow cron calls (Railway sets this header) or internal calls
  ...
  try {
    ...
    if (!refPrice) {
      return res.status(200).json({ status: 'no_reference', message: 'Not enough price history yet — check back in 4 hours' });
    }
    ...
    if (absMove < MOVE_THRESHOLD) {
      return res.status(200).json({ status: 'no_alert', movePct: (movePct * 100).toFixed(2) + '%', threshold: '5%' });
    }
    ...
    if (lastAlert && Date.now() - lastAlert.getTime() < COOLDOWN_MS) {
      ...
      return res.status(200).json({ status: 'cooldown', lastAlertHoursAgo: hoursAgo, movePct: (movePct * 100).toFixed(2) + '%' });
    }
    ...
    console.log(`Alert fired: ${direction} ${(movePct * 100).toFixed(1)}% — ${heading}`);
    return res.status(200).json({ status: 'alert_sent', direction, movePct: (movePct * 100).toFixed(2) + '%', heading });
